...
const ShowLetters = () => {
  ...
  const { letters, department, employees, users } = useContext(StoreContext);
  //getting letters
  ...
  const letter = new LettersClass(Letters, Users, Employees, department.state);
      ...
      return (
        <tr key={l._id}>
          <td className="row">
            <FontAwesomeIcon
              icon={faEnvelope}
              className="text-info my-auto mx-2"
            />
            <p className="text-info my-auto">{l.title}</p>
          </td>
          <td>
            {letter.isApproved(l._id) &&
            letter.isParticipation(l._id) &&
            letter.isAccepted(l._id) ? (
              <p className="text-success">Approved participation Letter</p>
            ) : letter.isAccepted(l._id) &&
              letter.isApproved(l._id) &&
              !letter.isParticipation(l._id) ? (
      ...
      return (
        <tr key={l._id}>
          <td className="row">
            <FontAwesomeIcon
              icon={faEnvelope}
              className="text-dark my-auto mx-2"
            />
          ...
          <td>
            <p className="text-info">
              {letter.Name(l.creater)}({letter.UserRole(l.creater)})
            </p>
          </td>
        </tr>
      );
          ...
          <div className="col-lg-12 ">
            <div className="main-card mb-3 min-height card">
              <h3 className="mt-5 text-center text-danger">
                ...
                <br />
                ...oops loading failed the server is down or not active contact
                admin
              </h3>
            </div>
          </div>
          ...
          <div className="container mt-5">
            <div className="row">
              <div className="col-lg-12 text-center">
                <FontAwesomeIcon
                  icon={faEnvelopeOpen}
                  className="text-secondary fa-4x"
                />
                <h3 className="text-center">You have no letters</h3>
              </div>
            </div>
          </div>
        )}
